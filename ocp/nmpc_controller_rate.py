"""
NMPC formulation for the quadrotor UAV (rate-controlled).

Builds an AcadosOcp with:
  - External cost  (position + quaternion-log + control effort)
  - Box constraints on controls  (thrust & angular velocity)
...
from config.experiment_config import (
    Q_POSITION, Q_ORIENTATION, R_CONTROL_RATE,
    T_MAX, T_MIN, W_MAX,
)
...
def create_ocp_solver_description(
    ...
    """Create the acados OCP description (rate-control model).
    ...
    # ── Cost matrices ─────────────────────────────────────────────────────
    Q_mat = np.diag(Q_POSITION)
    K_mat = np.diag(Q_ORIENTATION)
    R_mat = np.diag(R_CONTROL_RATE)
