...
The three terms in~\eqref{eq:pmm_cost} trade the final time $T_f$
against control effort ($w_a$) and jerk smoothness ($w_j$): the jerk
regulariser is what keeps the downstream flatness derivatives ---
the attitude quaternion and body rate of
Section~\ref{sec:flat_map}, obtained by differentiating the planned
acceleration --- well conditioned, so $w_j$ couples the planner cost
directly to the quality of the reference handed to the tracker.
...
These values follow standard practice and the real-time budget rather
than a separate tuning procedure: $f_c=\SI{100}{\hertz}$ is the common
control rate for body-rate agile \nmpc{}
\cite{hanover2021performance,verschueren2022acados}, and $N=50$
($T_h=\SI{0.5}{\s}$) is the largest horizon for which the present
implementation keeps the P99 solve time within the
\SI{10}{\milli\second} deadline on the test platform
(Section~\ref{sec:computation}); the resulting half-second look-ahead
spans roughly one inter-gate transit
($T_f^*/N_g\!\approx\!\SI{0.7}{\s}$), enough to anticipate the upcoming
gate without over-extending the horizon.
...
\begin{figure*}[!htbp]
  ...
  \caption{Evaluation circuits in 3-D.
           ...
           attitude reference.
           Black dashed line: \pmm{} reference; blue: \nmpc{}-Att;
...
attributable to the tail of the disturbance distribution) are flagged
as \emph{blow-ups} and excluded from the statistics under a paired rule
(a trial is dropped if \emph{either} controller diverges, so both are
evaluated on the same disturbance realisations), leaving $23/25$ valid
trials on the figure-8 and $18/25$ on the more demanding vertical loop.
