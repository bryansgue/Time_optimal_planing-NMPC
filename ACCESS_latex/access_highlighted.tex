...
\usepackage{soul}
\sethlcolor{yellow}
\let\st\undefined % soul defines \st; free it for the paper's \st (s.t.) macro
% Highlight macro for revised passages; \mbox-protect math/commands inside.
\newcommand{\hlt}[1]{\hl{#1}}
% Full-paragraph highlight (robust to math/citations); used for new paragraphs.
\newcommand{\hlpar}[1]{\par\noindent\colorbox{yellow}{%
  \parbox{\dimexpr\linewidth-2\fboxsep\relax}{#1}}\par}
\makeatletter
...
\author{{Bryan S. Guevara}\authorrefmark{1}, {Alexandre Santos Brandão}\authorrefmark{2}
...
(e-mail: bguevara@indoamerica.edu.ec)}

\address[2]{Núcleo de Especialização em Robótica, Programa de Pós-Graduação em Ciência da Computação, Departamento de Engenharia Elétrica, Universidade Federal de Viçosa, Viçosa 36570-900, MG, Brazil (e-mail: alexandre.brandao@ufv.br)}

\tfootnote{The research was funded by Universidad Tecnológica Indoamérica.}
...
GitHub: \url{https://github.com/bryansgue/Time_optimal_planing-NMPC}.
\end{abstract}
...
seconds.
Zhou~\textit{et~al.}~\cite{zhou2023timeoptimal} tackle the same
time-optimal problem directly on the full 13-D quadrotor dynamics and
close the loop with a time-adaptive MPC that adjusts the tracking
horizon online, at the cost of coupling planning and control into a
single monolithic formulation.
...
\hlt{The three terms in~\mbox{\eqref{eq:pmm_cost}} trade the final time
\mbox{$T_f$} against control effort (\mbox{$w_a$}) and jerk smoothness
(\mbox{$w_j$}): the jerk regulariser is what keeps the downstream
flatness derivatives --- the attitude quaternion and body rate of
Section~\mbox{\ref{sec:flat_map}}, obtained by differentiating the
planned acceleration --- well conditioned, so \mbox{$w_j$} couples the
planner cost directly to the quality of the reference handed to the
tracker.}
...
\hlt{These values follow standard practice and the real-time budget
rather than a separate tuning procedure: \mbox{$f_c=\SI{100}{\hertz}$}
is the common control rate for body-rate agile \mbox{\nmpc{}}
\mbox{\cite{hanover2021performance,verschueren2022acados}}, and
\mbox{$N=50$} (\mbox{$T_h=\SI{0.5}{\s}$}) is the largest horizon for
which the present implementation keeps the P99 solve time within the
\mbox{\SI{10}{\milli\second}} deadline on the test platform
(Section~\mbox{\ref{sec:computation}}); the resulting half-second
look-ahead spans roughly one inter-gate transit
(\mbox{$T_f^*/N_g\!\approx\!\SI{0.7}{\s}$}), enough to anticipate the
upcoming gate without over-extending the horizon.}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{Fig/Mujuco_sim.png}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lcc}
    ...
    $\max|\vect{\omega}^*|$ [\si{\radian\per\second}] & 9.00 & 9.00     \\
...
\begin{figure*}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{Fig/fig_pmm_circuits.pdf}
  \caption{Evaluation circuits in 3-D.
           ...
           attitude reference.
           \hlt{Black dashed line: \mbox{\pmm{}} reference; blue:
           \mbox{\nmpc{}}-Att; green: \mbox{\nmpc{}}-Full; red markers:
           gate centres.}}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{Fig/fig_flatness_analysis_loop.pdf}
...
\hlt{Trials in which either controller diverged
(\mbox{$\text{RMSE}>\SI{5}{\m}$}, attributable to the tail of the
disturbance distribution) are flagged as \mbox{\emph{blow-ups}} and
excluded from the statistics under a paired rule (a trial is dropped if
\mbox{\emph{either}} controller diverges, so both are evaluated on the
same disturbance realisations), leaving \mbox{$23/25$} valid trials on
the figure-8 and \mbox{$18/25$} on the more demanding vertical loop.
We report the gate-crossing rate alongside the continuous metrics.}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{Fig/fig_pos_error_combined.pdf}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{Fig/fig3_gate_error_loop.pdf}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{Fig/fig6_att_error_loop.pdf}
...
\hlpar{%
Positioned against the cited literature, the proposed pipeline occupies
a distinct point in the design space.
Foehn~\textit{et~al.}~\cite{foehn2021time} pair a \pmm{} planner with
\mbox{MPCC} tracking but do not feed a flatness-derived full-state
reference to the tracker; we retain their fast point-mass planning while
passing the complete flat output $(\vect{p},\vect{v},\vect{q},
\vect{\omega},T)$ to the \nmpc{}.
The \mbox{MPCC} formulations of
Romero~\textit{et~al.}~\cite{romero2022mpcc} and
Krinner~\textit{et~al.}~\cite{krinner2024mpcc} reach excellent crossing
rates but fuse planning and tracking into a single optimisation, so the
planner and controller cannot be designed, validated, or replaced
independently; our architecture keeps the two stages modular while
recovering the tracking completeness that the fused formulation
achieves implicitly.
Finally, Sun~\textit{et~al.}~\cite{sun2022comparative} benchmark \nmpc{}
against differential-flatness-based control over 76 trajectories but
hold the reference structure fixed, whereas the present study
\emph{ablates} that structure and isolates which flat components
(angular rate, tilt-corrected attitude) drive gate-crossing
performance.
The result is a pipeline with the modularity of decomposed approaches
and the reference completeness usually exclusive to fused ones, at no
additional solver cost.}
